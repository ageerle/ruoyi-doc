...
当前默认提供**开始、结束、生成回答、条件分支、网络搜索和知识检索**节点，可用于接收输入、生成内容、搜索资料和按条件选择执行路径。**图片生成、邮件发送和 HTTP 请求**也已有实现，需先初始化对应节点并配置依赖，具体条件见[当前版本的真实能力](#当前版本的真实能力)。
...
导入当前 `docs/script/sql/ruoyi-ai.sql` 后，组件表默认包含以下 6 个启用节点：
...
使用当前全量 SQL 初始化的环境已包含“知识库检索”节点。已有环境可执行后端仓库的 `docs/script/sql/update/2026-09-11-knowledge-retrieval-node.sql` 注册脚本，再刷新设计器，将节点拖入画布。该脚本只补充缺失记录；已有停用记录需单独启用。
...
| 检索模式 | `vector` | 可选向量或混合检索；节点选择覆盖知识库自身的混合检索设置 |
| 严格模式 | 开启 | 检索结果为空时使用下方的默认回复；关闭时输出空文本 |
| 默认回复 | `未检索到相关内容，请换一种方式提问。` | 严格模式下填写；留空不会自动生成兜底文案 |

属性面板默认返回数量为 `3`、相似度阈值为 `0.6`。后端还支持 `prompt` 查询改写和 `return_source` 来源输出配置，当前专用面板未提供这两个字段；来源输出默认开启。严格模式只处理本节点的空检索输出，不会终止流程，也不约束后续生成回答节点。检索异常也可能表现为空结果，排查时需结合后端日志。
...
当前基线已注册知识检索；旧环境按知识检索章节执行升级脚本。通义万相、邮件和 HTTP 节点可通过本页提供的初始化 SQL 启用。刷新页面，并确保组件名与执行类匹配。
...
- [ ] 基线组件库能看到开始、结束、生成回答、条件分支、网络搜索和知识检索。
